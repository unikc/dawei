...
export function ProductVisual({ type, image, name, compact = false }: { type: Product["type"]; image: string; name: string; compact?: boolean }) {
  return (
    <div className={`relative flex items-center justify-center overflow-hidden bg-gradient-to-br from-background to-muted ${compact ? "h-52" : "h-80"}`}>
      ...
      <span className="absolute bottom-3 right-4 bg-background/80 px-2 py-1 text-[9px] font-bold uppercase tracking-[.2em] text-muted-foreground">Dawei product range</span>
    </div>
  );
}
