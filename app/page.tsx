...
export default function Home() {
  return <>
    ...
    <section className="px-5 py-20"><div className="mx-auto max-w-7xl"><div className="mb-10 flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><p className="eyebrow">Complete product range</p><h2 className="section-title">Valves and piping products<br/> for international projects.</h2></div><Link href="/products" className="flex items-center gap-2 text-sm font-bold text-valve">Search all products <ArrowRight size={16}/></Link></div>
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{products.map((p,i)=><Card className={`group overflow-hidden hover:border-primary hover:shadow-lg ${i<2?"border-primary":""}`} key={p.slug}><Link href={`/products/${p.slug}`}><ProductVisual type={p.type} image={p.image} name={p.name} compact/><CardContent className="p-5">{i<2&&<Badge variant="signal">Core Focus</Badge>}<h3 className={`${i<2?"mt-3":"mt-0"} text-xl font-black text-navy group-hover:text-primary`}>{p.name}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{p.short}</p><span className="mt-4 flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-primary">View product series <ArrowRight size={14}/></span></CardContent></Link></Card>)}</div></div></section>
