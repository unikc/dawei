...
export default function ProductDetailPage({params}:{params:{slug:string}}){
  ...
  return <><div className="mx-auto grid max-w-[1480px] gap-5 px-4 py-5 lg:grid-cols-[270px_minmax(0,1fr)] lg:items-start">
    ...
    <main className="min-w-0 overflow-hidden border border-border bg-background">
      <section className="bg-muted px-5 py-8 md:px-8"><Link href="/products" className="text-xs font-bold uppercase tracking-wider text-primary">← All products</Link><div className="mt-5 grid gap-6 xl:grid-cols-2 xl:items-center"><div><h1 className="text-3xl font-black leading-tight text-navy md:text-4xl">{product.title}</h1><p className="mt-3 text-sm leading-6 text-muted-foreground">{product.summary || "Contact Dawei for product specifications and availability."}</p></div><div className="grid grid-cols-2 gap-2">{product.images.slice(0,4).map((src,i)=><ProductImageLightbox src={src} alt={`${product.title} image ${i+1}`} featured={i===0} key={src}/>)}{!product.images.length&&<Card className="col-span-2 grid h-56 place-items-center text-muted-foreground"><FileText size={42}/></Card>}</div></div></section>
